...
//получаем имя и сл этапы редактируемой олимпиады 
$row = mysql_fetch_array(mysql_query("SELECT name_olympiad, nextStage FROM olympics WHERE id='$id_o'"));

$row_name = $row['name_olympiad']; // старое имя олимпиады
$stage = $row['nextStage'];
...
while ($row = mysql_fetch_array($result)) {
	...
	if($row_name==$name_olimp){
		...
		$message = "Уважаем".$ending." ".$name[1]."!\nВ информации об олимпиаде под названием: ".$row_name." (".$name_olimp."), в которой Вы участвуете, произошли изменения. Просим Вас ознакомиться с данной информацией.\n\nС уважением, Администрация olimpiada.ru";
	}
...
if ($kol == $number_date) { //если кол-во этапов не изменилось
	// обновляем первый этап
	...
	// обновляем остальные этапы при необходимости
	if ($number_date > 1) { //всего один этап
		for($i=2; $i<=$number_date; $i++){
			...
			$j = $i - 2;
			mysql_query ("UPDATE olympics SET name_olympiad='$name_st', date='$date_time', location='$location_olimp', classes='$class_string', terms='$date_application', description='$description_olimp', subject='$subject', professor_users_id='$Org_olimp' WHERE id='$nextStage[$j]'");
		}
	} 
}
