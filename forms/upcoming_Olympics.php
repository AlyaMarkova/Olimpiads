...
$.ajax({
	...
	success: function(html){						
		...
		count_record = html.array_id.length; // количество записей
		...
		for(var i=0;i<array_id.length;i++){
			...
			if($('#'+i)[0]){
				...
				if(rights_user==0 || non_confirmed == -2){ // для гостей и неподтверждённых
					document.getElementById(i+'btn_edit').style.display="none"; // редактирования нет
					document.getElementById(i+'btn_delete').style.display="none"; // удаления нет
					document.getElementById(html.array_id[i]+'btn_zayvka2').style.display="none"; //подачи/отмены заявки нет
				...
				if (now3 > now4){ // если срок подачи заявки прошёл
					document.getElementById(html.array_id[i]+'btn_zayvka2').disabled=true;
					document.getElementById(html.array_id[i]+'btn_zayvka').disabled=true;
					document.getElementById(html.array_id[i]+'btn_zayvka2').title="Вы не можете подать заявку";
					document.getElementById(html.array_id[i]+'btn_zayvka').title="Вы не можете подать заявку";
...
function classes(olymp_class,user_class){ // определяет, подходит ли ученик по классу на олимпиаду
	var str=olymp_class;
	do {
		var from = str.search(','); 
		if (from != -1) { 
...
function periud(id){ // фильтр вывода олимпиад: за неделю, за месяц, все
	var now = new Date();
	if (id == "week"){
		now.setDate(now.getDate()+6); // получаем дату через неделю
		for(var i=0;i<date_vidod.length;i++){
			var now2 = new Date(document.getElementById(date_vidod[i]+'date').value); //получаем дату проведения олимпиады
			if(now < now2){ // если дата проведения олимпиады больше, чем через неделю, то не выводим эту запись
	...
	if(id=="month"){
		now.setMonth(now.getMonth()+1); // получаем дату через месяц
		for(var i=0;i<date_vidod.length;i++){
	...
	if(id=="all"){
		for(var i=0;i<date_vidod.length;i++){ // выводим всееее
			document.getElementById(date_vidod[i]).style.display="block";			
		}
		document.getElementById('week').setAttribute("class", "day_select");
		document.getElementById('month').setAttribute("class", "day_select");
...
function  create_div_olimpics(i,id, name, subject, classes, terms, date,id_org,status){
	...
	div_elem.style.margin = "0 0 5px 0px";	
	...
	do {
		...
		id_stage++;
	} while ((now>dat)&&(str.length>0)); // пока текущая дата больше даты этапа и пока не закончилась строка
	
	if(now>dat){ //если текущая дата больше даты последнего этапа
		var par2={
			"id": id,
			}
		$.ajax({
